...
function [ly, w, oldw] = bpnn_adjust_weights(delta, ndelta, ly, nly, w, oldw)
  ...
  for j = (mc_t107 : ndelta);
    ...
    for k = (mc_t106 : nly);
      mc_t100 = ETA;
      [mc_t101] = delta(j);
      [mc_t98] = mtimes(mc_t100, mc_t101);
      [mc_t99] = ly(k);
      [mc_t94] = mtimes(mc_t98, mc_t99);
      mc_t96 = MOMENTUM;
      [mc_t97] = oldw(k, j);
...
%sum
    ...
    for k = (mc_t80 : no);
      ...
      [mc_t71] = delta_o(k);
      [mc_t72] = who(j, k);
      [mc_t70] = mtimes(mc_t71, mc_t72);
      [s] = plus(mc_t69, mc_t70);
    end
    mc_t75 = h;
    mc_t81 = 1.0;
    [mc_t76] = minus(mc_t81, h);
    [mc_t73] = mtimes(mc_t75, mc_t76);
    mc_t74 = s;
    [mc_t68] = mtimes(mc_t73, mc_t74);
    delta_h(j) = mc_t68;
    mc_t77 = errsum;
    [mc_t79] = delta_h(j);
    [mc_t78] = abs(mc_t79);
